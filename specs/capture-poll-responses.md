...
- if poll is configured to no longer accept response, the should not be visible
- if poll is configured to require an email address, a field to capture guest email should be shown to capture the email. The email field now needs to be required
- if poll is configured to auto submit the vote, then the user no need to submit the response, it should use the answer and email parameter to auto submit the vote
